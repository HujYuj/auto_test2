...
class DpdinStatus():
    ...
    def DPDIN_test(self, repeat=1):
        current_path = os.path.dirname(os.path.realpath(__file__))
        try:
            with open(os.path.join(current_path, "CarrierTestResults/CarrierTestResult.json"), 'r') as f:
                res = json.load(f)
        ...
        with open(os.path.join(current_path, "CarrierTestResults/CarrierTestResult.json"), 'w') as f:
            json.dump(res, f)

        if self.fail_flag:
            download_runtime_log(path=os.path.join(current_path, 'MainTestResults/runtimeLog/'), log_name='carrier_fail')
