...
#        doubleParav = c_double(0.0)
    ...
    def PowerSuppy_Init(self,interface,address):
        ...
        if interface == "GPIB":
            ...
            try:
                PS_handler = self.resourceManager.open_resource("GPIB0::%s::INSTR"%(address))
                return PS_handler
            except Exception:
                return False
...
if __name__=="__main__":
    intr = visaInstrument()
    power_handler = intr.PowerSuppy_Init("TCPIP_VISA_RAWSOCKET", "192.168.254.11")
    # print(power_handler)
    intr.PowerSuppy_OFF_DELTA(power_handler)
#
