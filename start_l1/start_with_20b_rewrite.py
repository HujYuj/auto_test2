...
class StabilityTest(object):
    ...
    def one_cycle(self):
        ...
        self.power_read()
        """
        if dpdin_flag or jesd_flag or CpriFlag:
        ...
        if CpriFlag:
            ...
            if not CpriSyncHandler.Cpri_test(REPEAT=cpri_repeat_times):
        ...
        if jesd_flag:
            ...
            self.jesd_test_pass = JesdVerifyHandler.JesdTest(jesd_repeat_times)
            # self.set_pause_flag()
            self.check_if_pause()
        """
        """
        if dpdin_flag:  # or udhcp_flag or soap_flag
            DpdinCheckHandler = DpdinStatus(made_ips)
            self.dpdin_test_pass = DpdinCheckHandler.DPDIN_test(repeat=dpdin_repeat_times)
...
if __name__ == '__main__':
    runner_params = {'abil_slot': '3', 'bw': ['40'], 'dl_tm': 'TM1_1', 'ul_tm': 'A1-5', 'tdd_switch': '1116_3ms_a5',
                     'rru': 'AEQY',
                     'case_type': '1CC', 'freq_list': ['3650'], 'power_list': ['36'], 'cell_id_list': [],
                     'rnti_list': [], 'rboffset_list': [],
                     'power_backoff_list': [], 'jesd_flag': 'True', 'cpri_flag': 'True', 'dpdin_flag': 'True',
                     'cycle_times': '100', 'cpri_repeat_times': '3',
                     'jesd_repeat_times': '3', 'dpdin_repeat_times': '1',
                     'socket_io_server_url': {'url': 'http://localhost:8032', 'event': 'runner_message_event'},
                     'power_supply_model': 'TOE', 'power_supply_address': '15', 'serial_port': 'COM11',
                     'l1_version': '20B'}
