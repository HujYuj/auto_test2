...
try:
    from auto_test.start_l1.connections.miniftp import SCP
...
class JesdReader:
    ...
    def load_commands(self):
        ...
        if self.product_type == "AEQZ" or self.product_type == "AEQC":
    ...
    def filename_initiate(self, file_path):
        if file_path is None:
            file_path = os.path.join(os.path.dirname(current_path), "testings/JesdLogAndResults")

        time = datetime.datetime.strftime(datetime.datetime.now(), '%Y%m%d')  # _%H%M%S')
        self.filename = os.path.join(file_path, self.product_type + '_jesd_' + time + '.csv')
...
if __name__ == "__main__":
    ...
    jesdReader = JesdReader(product_type="AEQC", made_ips=made_ips)
